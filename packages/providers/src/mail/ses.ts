import {DnsRecord, DomainIdentity, EmailIdentity, Mailer, RawSesConfigSchema, SesConfig, VerifyResult} from "../core";
import {
    CreateReceiptRuleSetCommand, DeleteReceiptRuleCommand, DescribeActiveReceiptRuleSetCommand,
    GetSendQuotaCommand,
    ListReceiptRuleSetsCommand, ReceiptRule,
    SendEmailCommand,
    SESClient, SetActiveReceiptRuleSetCommand, SetReceiptRulePositionCommand, UpdateReceiptRuleCommand
...
import {
    ...
    CreateBucketCommandInput, BucketLocationConstraint, PutPublicAccessBlockCommand,
} from "@aws-sdk/client-s3";
import {
    SNSClient,
    CreateTopicCommand,
    GetTopicAttributesCommand,
    SetTopicAttributesCommand, ListSubscriptionsByTopicCommand, SubscribeCommand,
...
import {
    ...
    GetEmailIdentityCommand, DeleteEmailIdentityCommand, PutEmailIdentityMailFromAttributesCommand,
...
export class SesMailer implements Mailer {
    ...
    async verify(id: string, metaData?: Record<string, any>): Promise<VerifyResult> {
        try {
    ...
    private async ensureRuleSet(ses: SES, desired: string):
        Promise<{ name: string; usedExistingActive: boolean }> {
        const active = await ses.send(new DescribeActiveReceiptRuleSetCommand({}));
        if (active.Metadata?.Name) return { name: active.Metadata.Name, usedExistingActive: true };

        const sets = await ses.send(new ListReceiptRuleSetsCommand({}));
        const exists = sets.RuleSets?.some(r => r.Name === desired);
        if (!exists) {
            try { await ses.send(new CreateReceiptRuleSetCommand({ RuleSetName: desired })); }
            catch (e: any) { if ((e.name || e.Code) !== "RuleSetNameAlreadyExists") throw e; }
        }
        await ses.send(new SetActiveReceiptRuleSetCommand({ RuleSetName: desired }));
    ...
    async addEmail(address: string, metaData?: Record<string, any>): Promise<EmailIdentity> {
        const ses = new SES({ region: this.cfg.region, credentials: this.cfg });
        ...
        const ruleName = (slugify(`${normalized}`, {
            customReplacements: [
                ['@', ' at ']
            ]
        }) + `-${ulid()}`).slice(0, 64);
        ...
        await ses.send(new SetReceiptRulePositionCommand({
            RuleSetName: activeRuleSet,
            RuleName: ruleName
        }));
    ...
    async ensureWebhookSubscription(
        ...
        const same = existing.Subscriptions?.find(s => s.Endpoint === webhookUrl && s.Protocol?.startsWith("http"));
    ...
    private async bootstrap(id: string, metaData: Record<any, any>): Promise<BootResult> {
        ...
        const { bucket, topicName, ruleSetName, defaultRuleName } = this.baseNames(id);
        ...
        const { subscribed } = await this.ensureWebhookSubscription(sns, topicArn, `${metaData.WEB_URL}/api/v1/hooks/aws/sns`);
        console.log("subscribed", subscribed)

        // Create the default inbound rule if missing
        const { name: ruleSetNameInUse, usedExistingActive } = await this.ensureRuleSet(ses, ruleSetName);
        ...
        return { bucket, topicArn, ruleSetName: ruleSetNameInUse, bucketExists, topicExists, ruleCreated };
    }


    async addDomain(domain: string, mailFrom: string, incoming?: boolean): Promise<DomainIdentity> {
        ...
        try {
            ...
            if (err?.name !== "ConflictException" && err?.name !== "AlreadyExistsException") {
                // Return a minimal object but still surface the error in meta
                return {
                    domain,
                    status: "unverified" as any,
                    dns: [],
                    meta: { error: err?.name ?? "CreateEmailIdentityError", message: err?.message },
                };
            }
        }
    ...
    private async configureMailFrom(
        ...
        const info = await this.v2.send(new GetEmailIdentityCommand({ EmailIdentity: domain }));
    ...
    async removeDomain(domain: string): Promise<DomainIdentity> {
        ...
        try {
            await this.client.send(new DeleteEmailIdentityCommand({ EmailIdentity: d }));
        } catch (e: any) {
            const notFound =
    ...
    async verifyDomain(domain: string): Promise<DomainIdentity> {
        ...
        try {
            const info = await this.v2.send(new GetEmailIdentityCommand({ EmailIdentity: d }));
            ...
            const notFound = e?.name === "NotFoundException" || e?.$metadata?.httpStatusCode === 404;
            return {
                domain: d,
                status: "unverified" as any,
                dns: [],
                meta: { error: notFound ? "IdentityNotFound" : e?.name, message: e?.message },
            };
        }
    }

    async sendTestEmail(to: string, opts?: { subject?: string; body?: string }): Promise<boolean> {
        return true
        // try {
