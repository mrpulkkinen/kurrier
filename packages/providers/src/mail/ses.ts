...
export class SesMailer implements Mailer {
    ...
    async sendEmail(
        to: string[],
        opts: { subject: string; text: string, html: string, from: string, inReplyTo: string, references: string[], attachments?: { name: string; content: Blob, contentType: string}[], },
        ...
        try {
            ...
            //     }),
            // );
            return {MessageId: `<${MessageId}@${this.cfg.region}.amazonses.com>`, success: true};
        } catch (err) {
            console.error("SES sendTestEmail error:", err);
