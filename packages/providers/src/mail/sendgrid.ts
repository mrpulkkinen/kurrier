...
import {
    DomainIdentity,
    Mailer, RawSendgridConfigSchema,
    SendgridConfig,
    VerifyResult,
} from "../core";
...
export class SendgridMailer implements Mailer {
    ...
    async addDomain(domain: string, opts: Record<any, any>): Promise<DomainIdentity> {
        try {
            ...
            const body = res.body as {
                ...
                valid: boolean;
                dns: {
                    mail_cname: { valid: boolean; type: "cname"; host: string; data: string };
    ...
    async verifyDomain(domain: string, opts: Record<any, any>): Promise<DomainIdentity> {
        ...
        try {
            ...
            const match = all.find((row) => String(row.domain).toLowerCase() === domain);
            ...
            const status: IdentityStatus = validation.valid ? "verified" : "unverified";
