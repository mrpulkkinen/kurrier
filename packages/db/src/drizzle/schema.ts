...
export const threadsList = pgTable(
    "threads_list",
    {
        // 1:1 with threads.id (thread is already per-mailbox)
        id: uuid("id")
            .primaryKey()
            .references(() => threads.id, { onDelete: "cascade" }),
        ...
        subject: text("subject"),
        previewText: text("preview_text"), // ~200 chars max (enforced in app/trigger)
    ...
    (t) => [
        // hot path: mailbox list ordering
        index("ix_threads_list_mailbox_activity").on(t.mailboxId, t.lastActivityAt, t.id),

        index("ix_threads_list_identity_public_id").on(t.identityPublicId),

        // reduce joins when resolving by identity+slug
        index("ix_threads_list_identity_slug").on(t.identityId, t.mailboxSlug),
        ...
        // RLS: mirror your pattern
        pgPolicy("threads_list_select_own", {
            for: "select",
            to: authenticatedRole,
            using: sql`${t.ownerId} = ${authUid}`,
        }),
        pgPolicy("threads_list_insert_own", {
            for: "insert",
            to: authenticatedRole,
            withCheck: sql`${t.ownerId} = ${authUid}`,
        }),
        pgPolicy("threads_list_update_own", {
            for: "update",
            to: authenticatedRole,
            using: sql`${t.ownerId} = ${authUid}`,
            withCheck: sql`${t.ownerId} = ${authUid}`,
        }),
        pgPolicy("threads_list_delete_own", {
            for: "delete",
            to: authenticatedRole,
            using: sql`${t.ownerId} = ${authUid}`,
        }),
