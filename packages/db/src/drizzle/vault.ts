...
async function vaultDeleteSecret(
    tx: PgTransaction<any, any, any>,
    id: string,
) {
...
export async function getSecret(
    ...
    const value = await admin.transaction((tx) => vaultGetSecret(tx, meta.vaultSecret));
...
export async function updateSecret(
    ...
    if (input.value !== undefined) {
        await admin.transaction((tx) => vaultUpdateSecret(tx, meta.vaultSecret, input.value!));
    }
