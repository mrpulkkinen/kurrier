...
export const syncProviders = async () => {
    ...
    await rls((tx) => tx.insert(providers)
        .values(PROVIDERS.map((k) => ({ type: k.key })))
        .onConflictDoNothing({ target: [providers.ownerId, providers.type] })
        .returning());
    const rows = await rls((tx) =>
        tx.select().from(providers).leftJoin(providerSecrets, eq(providerSecrets.providerId, providers.id))
    )
    return rows;
};
...
export type ProviderSecretsResult = Awaited<ReturnType<typeof getProviderSecrets>>;
export type ProviderSecretRow = ProviderSecretsResult[number];
export type SyncProvidersRow = Awaited<ReturnType<typeof syncProviders>>[number];
...
export async function saveProviderEnv(
    ...
    try {
        ...
        const {keys, providerId}: {keys: Record<string, string>, providerId: string} = decode(formData)
        const session = await currentSession()
        ...
        for (const key in keys){
            const [currentProviderSecret] = await rls((tx) => {
                return tx.select().from(providerSecrets).where(and(
            ...
            if (currentProviderSecret) {
                ...
                await rls((tx) => tx.insert(providerSecrets).values({
                    providerId,
