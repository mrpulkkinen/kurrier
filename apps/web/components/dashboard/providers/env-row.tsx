...
export default function EnvRow({
    ...
    return (
        <div className="flex flex-col gap-2 rounded-lg border p-3 sm:p-4">
            ...
            <div>
                 <Input defaultValue={secret?.vault?.decrypted_secret || ""} onFocus={() => setHide(false)} onBlur={() => setHide(true)} required={true} name={name} type={!hide ? "text" : "password"} className="h-10 w-full text-base" />
            </div>
        </div>
    );
}
