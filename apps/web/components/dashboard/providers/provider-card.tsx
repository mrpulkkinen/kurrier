...
import {ProviderSecretRow, ProviderSecretsResult, saveProviderEnv, SyncProvidersRow} from "@/lib/actions/dashboard";
...
export default function ProviderCard({ spec, userProvider, secrets }: { spec: ProviderSpec, userProvider: SyncProvidersRow, secrets: ProviderSecretsResult }) {
    ...
    const envStatuses = spec.requiredEnv.map((name) => ({
        ...
        secret: secrets.find(s => s.providerSecret?.keyName === name) as ProviderSecretRow,
    }));
    ...
    const [formState, formAction, isPending] = useActionState<FormState, FormData>(saveProviderEnv, {});
            ...
            <Card className="shadow-none relative">
                ...
                <CardContent className="space-y-4 mb-16">
                    ...
                    <div className="space-y-3">
                        <input type={"hidden"} name={"providerId"} value={userProvider.providers.id} />
                        {envStatuses.map((row) => (
                            <EnvRow key={row.name} name={`keys.${row.name}`} rowName={row.name} present={row.present} secret={row.secret} />
                        ))}
                    </div>
                ...
                <CardFooter className={"flex justify-end absolute bottom-4 w-full flex-col gap-2"}>
                    {formState.error && (
