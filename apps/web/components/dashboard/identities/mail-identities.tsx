...
export default function MailIdentities({userIdentities, smtpAccounts, providerAccounts, providerOptions}: {
    ...
    const initTestEmail = async (userIdentity: FetchUserIdentitiesResult[number], decryptedSecrets: Record<any, unknown>) => {
        setSendTesting(true);
    ...
    const openAddEmailForm = async () => {
        const openModalId = modals.open({
            title: (
                <div className="font-semibold text-brand-foreground">Add Email Identity</div>
            ),
            closeOnEscape: false,
    ...
    const confirmDeleteIdentity = async (userIdentity: FetchUserIdentitiesResult[number]) => {
        modals.openConfirmModal({
            title: <div className={"font-semibold text-brand-foreground"}>Delete Identity</div>,
            centered: true,
            children: (
                <div className="text-sm ">
                    Are you sure you want to delete <b>{userIdentity.identities.value}</b>? This
                        ...
                        <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="grid w-full max-w-[560px] grid-cols-1 gap-3 sm:grid-cols-[1fr_auto]">
                                ...
                                <div className="flex gap-2">
                                    <Button className="w-full gap-2 sm:w-auto" aria-label="Add identity">
...
function EmptyState({ kind, query }: { kind: "domain" | "email"; query: string }) {
    ...
    return (
        <div className="rounded-lg border border-dashed p-6 text-center">
            <p className="text-sm text-muted-foreground">
                {searching ? (
                    <>
                        No {label} match <span className="font-medium text-foreground">“{query}”</span>. Try a different search.
                    </>
                ) : (
