...
import {FetchUserIdentitiesResult, getIdentityById} from "@/lib/actions/dashboard";

function EmailIdentityStatus({userIdentity}: {userIdentity: FetchUserIdentitiesResult[number]}) {

    const [incoming, setIncoming] = useState<boolean>(false);
    const evaluateStatus = async () => {
        if (userIdentity.identities.domainIdentityId){
            const domain = await getIdentityById(userIdentity.identities.domainIdentityId)
