...
const EmailEditor = dynamic(() => import("@/components/mailbox/default/editor/email-editor"), {
    ssr: false,
    loading: () => <div className="py-10 text-sm text-muted-foreground">Loading editor…</div>,
});
...
function getScrollParent(el: HTMLElement): HTMLElement {
    ...
    while (p) {
        ...
        const canScrollY =
            (s.overflowY === "auto" || s.overflowY === "scroll") && p.scrollHeight > p.clientHeight;
...
function scrollToEditor(
    ...
    setTimeout(() => {
        ...
        if (bottomGap < desiredBottomGap) {
            if (isWindow) {
                window.scrollBy({ top: desiredBottomGap - bottomGap, behavior: "smooth" });
            } else {
...
function EmailRenderer({message, children}: {message: MessageEntity, children?: React.ReactNode}) {
            ...
            <div className={"flex justify-between"}>
                <div>
                    <div className={"mt-4 flex gap-1 items-center"}>
                        <div className={"text-sm font-semibold capitalize"}>{message.fromName ? message.fromName : slugify(String(message.fromEmail), {separator: ' '})}</div>
                        <div className={"text-xs"}>{`<${message.fromEmail ?? message.fromEmail}>`}</div>
                    </div>
