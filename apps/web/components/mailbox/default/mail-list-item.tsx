...
import {Archive, Mail, MailOpen, MoreHorizontal, Paperclip, Star, StarOff, Trash2} from "lucide-react";
import {MailboxEntity, MessageEntity} from "@db";
import {useRouter} from "next/navigation";

function MailListItem({message, activeMailbox, identityPublicId}: {message: MessageEntity, activeMailbox: MailboxEntity, identityPublicId: string}) {

    const router = useRouter()
    const onMessageClick = async () => {
        router.push(`/mail/${identityPublicId}/${activeMailbox.slug}/${message.id}`)
    };
    ...
    return <>
        ...
        >
            ...
            <div
                ...
                // onClick={() => onOpenMail?.(m.id)}
            >
                <span className={["truncate", !message.seen ? "font-semibold" : ""].join(" ")}>
                  {message.subject}
                </span>
