...
export default function WebmailListItemMobile({
    ...
    const dateLabel = (() => {
        ...
        try {
            ...
            if (diff === 0)
                return zdt.toLocaleString(undefined, { hour: "numeric", minute: "2-digit" });
            if (d.year === today.year)
                return zdt.toLocaleString(undefined, { month: "short", day: "numeric" });
    ...
    const openThread = () =>
        router.push(`/mail/${identityPublicId}/${activeMailbox.slug}/threads/${mailboxThreadItem.threadId}`);
    ...
    function names(p: typeof mailboxThreadItem.participants) {
        ...
        const toText = (x: { n?: string | null; e: string }) => (x.n && x.n.trim()) || x.e;
    ...
    const { state, setState } = useDynamicContext<{ selectedThreadIds: Set<string> }>();
    ...
    return (
        ...
        >
            ...
            <div className="flex items-start gap-2 pt-1">
                <input
                    ...
                    onChange={(e) => {
                        ...
                        e.target.checked ? next.add(mailboxThreadItem.threadId) : next.delete(mailboxThreadItem.threadId);
            ...
            <div className="ml-auto flex flex-col items-end justify-start gap-1 text-right">
                <time className="whitespace-nowrap text-sm text-foreground">{dateLabel}</time>
            </div>
