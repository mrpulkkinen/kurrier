...
async function deleteUids(client: ImapFlow, mailboxPath: string, uids: number[]) {
...
export async function deleteMail(
    ...
    const [box] = await db.select().from(mailboxes).where(eq(mailboxes.id, mailboxId));
    if (!box) return;
    ...
    const msgRows = await db
        ...
        .from(messages)
        .where(
            and(eq(messages.mailboxId, mailboxId), inArray(messages.threadId, threadIds))
        );
