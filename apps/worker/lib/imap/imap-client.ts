...
function safeReconnect(identityId: string, imapInstances: Map<string, ImapFlow>) {
    const existing = imapInstances.get(identityId);
    if (existing) {
        try { existing.logout(); } catch {}
        imapInstances.delete(identityId);
    }
    setTimeout(() => initSmtpClient(identityId, imapInstances).catch(console.error), 5000);
}

export const initSmtpClient = async (identityId: string, imapInstances: Map<string, ImapFlow>) => {
    if (
        ...
        const noopInterval = setInterval(async () => {
            if (client.usable) {
                try { await client.noop(); } catch (err) {
        ...
        const cleanup = (reason: string) => {
            ...
            console.warn(`[IMAP:${identityId}] Disconnected (${reason}), reconnecting...`);
