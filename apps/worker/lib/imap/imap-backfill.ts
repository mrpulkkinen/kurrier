...
import {
    ...
    mailboxes,
    MailboxInsertSchema,
    mailboxSync, MailboxSyncInsertSchema, type MailboxUpdate, MailboxUpdateSchema
} from "@db";
    ...
    if (!remoteTrashPath) {
        try {
            ...
            const existing = locals.find(
                (l) => (l?.metaData?.imap?.path as string | undefined) === remoteTrashPath,
            );
            ...
            const localTrash = locals.find((l) => l.slug === "trash" || l.kind === "trash");
            if (!localTrash) {
                const parsedData = MailboxInsertSchema.parse({
                    ownerId: identity.ownerId,
                    identityId: identity.id,
