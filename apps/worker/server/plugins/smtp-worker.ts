...
import { getPublicEnv, getServerEnv, mailboxKindsList } from "@schema";
import {
    db,
    decryptAdminSecrets,
    identities,
...
export default defineNitroPlugin(async (nitroApp) => {
    ...
    backfillChannel
        ...
        .on("broadcast", { event: "delta" }, async ({ payload }) => {
            ...
            const mailboxRows = await db
                .select()
                .from(mailboxes)
                .where(eq(mailboxes.identityId, payload.identityId));
            ...
            for (const row of mailboxRows) {
                ...
                const [syncRow] = await db
                    ...
                    .where(
                        and(
                            eq(mailboxSync.identityId, payload.identityId),
                            eq(mailboxSync.mailboxId, row.id),
                        ),
                    );
                if (!syncRow) continue;
    ...
    async function syncMailbox(opts: {
        ...
        if (!sync) throw new Error(`mailbox_sync row missing for mailboxId=${mailboxId}`);
...
//     db,
//     decryptAdminSecrets,
//     identities, IdentityEntity, MailboxCreate,
//     mailboxes, MailboxInsertSchema, mailboxSync,
//     MailboxSyncInsertSchema, MailboxUpdate, MailboxUpdateSchema,
//     smtpAccountSecrets,
// } from "@db";
// import {and, eq, inArray, sql} from "drizzle-orm";
// import slugify from "@sindresorhus/slugify";
...
//                 path: String(row?.metaData?.imap.path),
//                 window: 500,
//                 onMessage: async (msg: FetchMessageObject, path: string, identityId: string, mailboxId: string) => {
//                     const raw = await msg?.source?.toString() || "";
//                     await parseAndStoreEmail(raw, {
//                         ownerId,
//                         mailboxId,
...
//         window?: number;              // batch size (default 500)
//         politeWaitMs?: number;        // small delay between batches (default 20ms)
//         onMessage: (msg: FetchMessageObject, path: string, identityId: string, mailboxId: string) => Promise<void>;
//     }) {
//         const {
//             client, identityId, mailboxId, path,
//             window = 500,
//             politeWaitMs = 50,
//             onMessage,
//         } = opts;
...
//             .select()
//             .from(mailboxSync)
//             .where(and(eq(mailboxSync.identityId, identityId), eq(mailboxSync.mailboxId, mailboxId)));
//
//         if (!sync) {
//             throw new Error(`mailbox_sync row missing for mailboxId=${mailboxId}`);
//         }
...
//         }
//
//         const topAtFinish = Math.max(0, (head.uidNext ?? 1) - 1);
//
//         // Mark phase explicitly (optional, harmless if already set)
//         await db.update(mailboxSync)
//             .set({ phase: "BACKFILL", lastSeenUid: topAtFinish, updatedAt: new Date() })
//             .where(eq(mailboxSync.id, sync.id));
//
//         while (cursor > 0) {
//             const end = cursor;
...
//                 internalDate: true,
//                 size: true,
//                 source: true,            // raw RFC822; remove if you store source elsewhere later
//             })) {
//                 // Let the caller persist (EML to storage + metadata to DB)
//                 await onMessage(msg, path, identityId, mailboxId);
//             }
//
//             // Move cursor backward and persist
...
//
//
//     const syncMailboxEntities = async (client: ImapFlow, identity: IdentityEntity): Promise<void> => {
//
...
//             touched.push(path);
//
//             const local = locals.find(l => (l?.metaData?.imap?.path as string | undefined) === path);
//
//             const meta = {
//                 ...(local?.metaData ?? {}),
//                 imap: {
...
//                     metaData: meta,
//                 })
//                 const [newMailbox] = await db.insert(mailboxes).values(parsedData as MailboxCreate).returning();
//                 const box = await client.mailboxOpen(path, { readOnly: true });
//                 const uidValidity = box.uidValidity!;
//                 const backfillCursorUid = Math.max(0, (box.uidNext ?? 1) - 1);
//
...
//                     updatedAt: new Date(),
//                 }) as MailboxUpdate
//                 await db.update(mailboxes).set(parsedData).where(eq(mailboxes.id, local.id));
//             }
//         }
//
//         // mark vanished as non-selectable
...
//         await client.mailboxOpen(path, { readOnly: true });
//
//         const [sync] = await db.select().from(mailboxSync)
//             .where(and(eq(mailboxSync.identityId, identityId), eq(mailboxSync.mailboxId, mailboxId)));
//         if (!sync) throw new Error(`mailbox_sync row missing for mailboxId=${mailboxId}`);
//
//         let lastSeen = Number(sync.lastSeenUid || 0);
