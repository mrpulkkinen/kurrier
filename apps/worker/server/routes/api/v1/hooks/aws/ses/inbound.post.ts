...
import {db, decryptAdminSecrets, mailboxes, MessageCreate, messages, providerSecrets} from "@db";
...
export default defineEventHandler(async (event) => {
    try {
        ...
        if (sns?.Type === "Notification" && sns?.Message) {
            ...
            const [inbox] = await db.select().from(mailboxes).where(and(
            ...
            const message = await db.insert(messages).values(messagePayload).returning()
